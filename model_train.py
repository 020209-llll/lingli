...
class VisualizationGenerator:
    ...
    def create_tsne_visualization(self):
        ...
        # t-SNE降维
        tsne = TSNE(n_components=2, random_state=42, perplexity=min(30, self.n_samples - 1))
        embeddings_2d = tsne.fit_transform(embeddings_pca)

        # 分离ATAC和RNA的坐标
        atac_2d = embeddings_2d[:self.n_samples]
        rna_2d = embeddings_2d[self.n_samples:]
    ...
    def plot_simple_visualizations(self, save_path="simple_visualizations.png"):
        """创建简化的可视化图，只包含t-SNE和PCA"""
        fig = plt.figure(figsize=(15, 6))

        # 1. t-SNE可视化
        atac_tsne, rna_tsne = self.create_tsne_visualization()

        # 2. PCA可视化
        atac_pca, rna_pca = self.create_pca_visualization()

        # 绘制t-SNE结果
        ax1 = plt.subplot(1, 2, 1)
        self._plot_modality_comparison(atac_tsne, rna_tsne, ax1, "t-SNE Visualization")

        # 绘制PCA结果
        ax2 = plt.subplot(1, 2, 2)
        self._plot_modality_comparison(atac_pca, rna_pca, ax2, "PCA Visualization")

        plt.tight_layout()
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
        plt.show()

        print(f"简化可视化图已保存至: {save_path}")

    def _plot_modality_comparison(self, atac_2d, rna_2d, ax, title):
        """绘制模态比较图"""
        # 计算质心
        atac_center = np.mean(atac_2d, axis=0)
        rna_center = np.mean(rna_2d, axis=0)

        # 绘制散点
        scatter1 = ax.scatter(atac_2d[:, 0], atac_2d[:, 1], alpha=0.7,
                              c='red', label='ATAC', s=30)
        scatter2 = ax.scatter(rna_2d[:, 0], rna_2d[:, 1], alpha=0.7,
                              c='blue', label='RNA', s=30)

        # 绘制质心
        ax.scatter(atac_center[0], atac_center[1], c='darkred',
                   marker='*', s=200, label='ATAC Center')
        ax.scatter(rna_center[0], rna_center[1], c='darkblue',
                   marker='*', s=200, label='RNA Center')
        ...
        # 添加椭圆显示分布范围
        self._add_confidence_ellipse(atac_2d, ax, 'red')
        self._add_confidence_ellipse(rna_2d, ax, 'blue')

    def _add_confidence_ellipse(self, points, ax, color, n_std=2.0):
        """添加置信椭圆"""
...
def main():
    ...
    with torch.no_grad():
        ...
        # 生成简化可视化
        print("\n生成简化可视化图...")
        visualizer = VisualizationGenerator(all_atac_emb, all_rna_emb, all_names, test_metrics)
        visualizer.plot_simple_visualizations("moco_test_set_simple_visualizations.png")
